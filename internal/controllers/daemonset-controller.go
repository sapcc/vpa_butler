...
func (v *VPADaemonsetController) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	var daemon = new(appsv1.DaemonSet)
	if err := v.Get(ctx, req.NamespacedName, daemon); err != nil {
		return ctrl.Result{}, client.IgnoreNotFound(err)
	}

	result, err := common.ReconcileVPA(ctx, v.Client, v.scheme, daemon, daemon.Spec.Template.Spec.Containers)
	if err != nil {
		return ctrl.Result{}, err
	}
	switch result {
	case common.OperationResultCreated, common.OperationResultUpdated:
		v.log.Info(fmt.Sprintf("VPA for DaemonSet was %s", result), "namespace", daemon.Namespace, "name", daemon.Name)
	}
